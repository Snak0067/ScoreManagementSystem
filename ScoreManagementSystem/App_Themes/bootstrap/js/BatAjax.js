...
function dispDdlBean(data) {
    var objId = data.dispControl;
    if ((data.datalist == null && typeof data.datalist == 'object')
        || (data.datalist == "" && typeof data.datalist == 'object')) {
        BatAjax.removeAllOptions(objId);
        return false;
    }
    if ($(objId) && $(objId).tagName.toLowerCase() == "select") {
        BatAjax.removeAllOptions(objId);
        BatAjax.addOptions(objId, data.datalist, data.valueLine, data.textLine);
        if ($(objId).options[0].value == "null") {
            $(objId).options[0].value = "";
        }
    }
    if (data.initControl) {
        if ($(data.initControl)) {
            if ($(data.initControl).value) {
                DWRUtil.setValue(objId, $(data.initControl).value);
            }
        }
    } else if (data.initValue) {
        DWRUtil.setValue(objId, data.initValue);
    }
}